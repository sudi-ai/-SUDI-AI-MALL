...
/**
 * Builds outfit suggestions from products that actually exist in CRMEB.
 */
class SudiAiOutfitService
{
    ...
    public function recommend(string $anchor, array $categories = []): array
    {
        ...
        foreach (array_slice($categories, 0, 4) as $category) {
            $keyword = trim($anchor . ' ' . (string)$category);
            $result = $this->search->search($keyword, 1, 4);
            $groups[] = ['category' => (string)$category, 'products' => $result['list']];
        }
        return [
            'anchor' => $anchor,
            'groups' => $groups,
